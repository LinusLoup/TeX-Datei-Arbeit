...
\begin{align*}
	...
	(\mcal N^+\setminus \mcal N^{++})  \cup (\mcal N\cap \partial \Omega) & \cup \underbrace{(\mcal N^0 \setminus (\mcal N^{0-} \cup \mcal N^{0+}))  \cup\mcal N^{0+}}_{=\mcal N^0\setminus \mcal N^{0-}} = \mcal N \setminus (\mcal N^{0-}\cup \mcal N^{++}) \, .
\end{align*}
Verwenden wir nun die sechs gezeigten Fälle, so ergibt sich:
\begin{align*}
	\rho_{\mcal S}(e) = & \sum_{p \in \mcal N} \rho_p(e) \\
	\le & \sum_{E\in \mcal E^0} d_E\, \abs{\rho_E} +\sum_{p \in \mcal N\setminus\mcal N^{0-}} 
\end{align*}
\end{proof}
