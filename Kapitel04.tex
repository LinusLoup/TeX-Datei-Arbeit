...
\[
	\mscr I (\bs v) = \frac 12 a(\bs v,\bs v) - \rho_{\mscr S} (\bs v) \text{ mit } \rho_{\mscr S} (\bs v) = a(\bs u_{\mscr S},\bs v)-F(\bs v) \, , 
\]
...
\end{align}
Jetzt lässt sich \eqref{eq:4.89} analog zu der Gleichung \eqref{eq:4.18} auch als Summe über die Kanten $E\in \mcal E$ beschreiben. Wenn wir $\bs n$ wieder als die Einheitsnormale bezeichnen, die von einem gegebenen Dreieck $T_1$ in das Dreieck $T_2$ zeigt, dann lässt sich der \textit{Nomalenspannungsfluss} über $E$ berechnen als
\[
...
\[
	\rho_p (\bs v) = \int_{\omega_p} \bs b \cdot \bs v \, \phi_p \, d\Omega + \int_{\scriptsize\parbox{1cm}{$E \subset \Gamma_\sigma$ \\ $E \in \mcal E_p$}} \bs t \cdot \bs v\, \phi_p \, d\Gamma + \sum_{E \in \mcal E_p} \int_E \bs v \cdot \bs j_E\, \phi_p \, d\Gamma \, ,
\]
wobei $\omega_p$ und $\mcal E_p$ wie oben definiert sind.
...
Es sei hier angemerkt, dass nun nicht mehr die inneren Knoten betrachtet werden wie in Definition \ref{def:4.16}, sondern alle Knoten aus $\mcal N$, da diejenigen Knoten, die in Kontakt stehen können, in den von uns betrachteten Problemen immer auf dem Rand des Gebietes $\Omega$ liegen.
...
\begin{align*}
	\mscr N^{++} & \coloneqq \{ p \in \mscr N^+ \mid (\eps_{\mscr V})_n (x_E) <g(x_E)-(u_{\mscr S}) \, \forall \, E \in \mcal E_p , E \subset \Gamma_c\} \, , \\
