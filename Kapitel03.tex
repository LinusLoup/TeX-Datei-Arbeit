...
\begin{align}\label{eq:3.38}
	\div\bs \sigma + \bs b = \bs 0 \, .
\end{align}

\item weiter gilt nach dem \textit{\idx{Cauchy-Theorem}} (vgl. auch \cite{WriggersContact} S. 38), dass die Spannung in Normalenrichtung auf der Oberfläche $\Gamma$ von $\Omega$ gleich der von außen angebrachten Spannung $\bs t$ ist, d.h.
\begin{align}\label{eq:3.39}
	\bs \sigma \cdot \bs n = \bs t \ \text{ auf } \Gamma_\sigma \, ,
\end{align}
...
\begin{itemize}
\item Sei $\Omega \subset \R^3$

\item wir betrachten das \idx{Signorini-Kontakt}-Problem \eqref{eq:3.41}

\item Es seien $H^1_{\Gamma_u}(\Omega) \coloneqq \{\bs v \in (H^1(\Omega))^3 \with \bs v = \bs 0 \text{ auf } \Gamma_u\}$ und $K \coloneqq \{\bs v \in H^1_{\Gamma_u}(\Omega) \with v_n - g \le 0 \text{ auf } \Gamma_c\}$ $\Ra$ $K$ ist analog zu Lemma \ref{lem:3.1} konvex

\item weiter seien $\bs u, \bs v \in K$, wobei $\bs u$ die Lösung des Signorini-Kontaktproblems darstellt und $\bs v$ (häufig in den Ingenieurswissenschaften als \textit{\idx{virtuelle Verschiebung}} bezeichnet) eine beliebige Testfunktion ist. Dann gilt $\bs w = \bs v - \bs u \in (H^1_{\Gamma_u}(\Omega))^3$ eine Testfunktion, die wir mit \eqref{eq:3.41a} multiplizieren und über $\Omega$ integrieren.
\begin{align}\notag
	 0 & =  \int_\Omega (\div \bs\sigma + \bs b) \cdot \bs w \, d \Omega = \int_\Omega \div \bs \sigma \cdot \bs w + \bs b \cdot \bs w \, d\Omega \\
	 \notag
	& = \int_\Omega \div(\bs w \cdot \bs\sigma) - \grad \bs w : \bs \sigma + \bs b \cdot \bs w \, d\Omega \\
	\notag
	& = \int_\Gamma \underbrace{\bs w \cdot \bs \sigma \cdot \bs n}_{=\bs w \cdot \norm{\bs n}^2 \cdot (\bs \sigma \cdot \bs n)} \, d\Gamma - \int_\Omega \underbrace{\frac 1 2 (\grad \bs w + \grad^T \bs w)}_{= \bs \eps (\bs w)} : \bs \sigma \, d\Omega + \int_\Omega \bs b \cdot \bs w \, d \Omega \\
	\label{eq:3.43}
	& = \int_{\Gamma_c} w_n \, \sigma_n \, d \Gamma + \int_{\Gamma_\sigma} \bs t \cdot \bs w \, d\Gamma-\int_\Omega \bs\sigma: \bs \eps(\bs w) \, d\Omega + \int_\Omega \bs b \cdot \bs w \, d\Omega
\end{align}
(zweite Zeile ist Produktregel für die Divergenz, dritte Zeile Gauß, vierte Zeile die Aufteilung von $\Gamma = \Gamma_u \cup \Gamma_\sigma \cup \Gamma_c$, wobei $\bs w = \bs 0$ auf $\Gamma_u$ und $\bs \sigma \bs n = \bs t$ auf $\Gamma_\sigma$).
...
\begin{align}\notag
	& 0 \ge \int_{\Gamma_\sigma} \bs t \cdot \bs w \, d\Gamma-\int_\Omega \bs\sigma: \bs \eps(\bs w) \, d\Omega + \int_\Omega \bs b \cdot \bs w \, d\Omega \\
	\label{eq:3.44}
	\Llra& \int_\Omega \bs\sigma(\bs u): \bs \eps(\bs v - \bs u) \, d\Omega \ge\int_\Omega \bs b \cdot (\bs v-\bs u) \, d\Omega+ \int_{\Gamma_\sigma} \bs t \cdot (\bs v-\bs u) \, d\Gamma 
\end{align}
...
\begin{align*}
	a(\bs u,\bs v) &\coloneqq \int_\Omega \bs\eps (\bs u) : \mcal C : \bs\eps(\bs v) \, d\Omega \, ,\\ 
	 F(\bs v)& \coloneqq \int_\Omega \bs b \cdot \bs v \, d\Omega + \int_{\Gamma_\sigma} \bs t \cdot \bs v \, d\Gamma 
\end{align*}
lässt sich \eqref{eq:3.44} in der altbekannten Form: Finde $\bs u \in K$, so dass gilt
\begin{align}\label{eq:3.45}
	a(\bs u, \bs v - \bs u) \ge F(\bs v- \bs u) \quad \forall \, \bs v \in K \, .
\end{align}
...
\item vgl. \cite{KikOden} S. 112, zeige: $\abs{F(\bs v)} \le C(\norm{\bs b}_0+\norm{\bs t}_{0,\Gamma_c})\, \norm{\bs v}_1$, also mit $\bs b \in (L^2(\Omega))^3$ und $\bs t \in (L^2(\Gamma_c))^3$.
...
\item 
\begin{theorem}
Es sei $\bs v : \Omega \ra \R^3$ und $K$ wie oben definiert. Die Variationsungleichung \eqref{eq:3.45} ist äquivalent zum Minimierungsproblem:
\begin{align}
...
\begin{itemize}
\item betrachte analog zu $\mcal S_h$ den Raum der linearen Ansatzfunktionen
