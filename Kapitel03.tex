...
Es lässt sich folglich auch eine a priori Abschätzung für den Fehler von $u$ und $u_h$ herleiten. Die Herleitung ist detailliert beispielsweise in \cite{Falk} wiederzufinden.
...
\end{align}
Damit gilt auch, wenn die Kontaktbedingung nicht aktiv ist (also nicht die Gleichheit gilt), muss Kräftegleichgewicht herrschen, d.h. in \eqref{eq:3.35} gilt die Gleichheit. Zusammen erhält man die \textit{\idx{Komplementaritätsbedingung}}
...
\end{align*}
Also ist $F$ unter der Voraussetzung, dass $\bar{\bs b}  \in (L^2(\Omega))^2$ und $\bar{\bs t} \in (L^2(\Gamma_c))^2$ gilt (analog zum Modellproblem aus Kapitel \ref{kap:2.2}), stetig. Damit folgt die Aussage des nächsten Theorems.
...
\begin{align*}
	\mscr K_{\mscr S}  &\coloneqq \bigg\{ \bs x \in \R^{2N} \,\bigg| \, \sum_{j=1}^{2N} x_j \, \bs \psi_j(\bar x_i) \cdot \bs n(\bar x_i) - g(x_i) \le 0 \text{ für } \bar x_i \in \mcal N \cap \Gamma_c\bigg\} \\
