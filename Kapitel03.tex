...
\begin{bem}
Ein Kontaktmodell mit $\bs \sigma_t \not = \bs 0$ ist das Modell mit \textit{\idx{Tresca-Reibung}}. Hierbei wird die letzte Bedingung aus \eqref{eq:3.41e} durch die Bedingungen
\begin{align}\label{eq:3.42}
	\norm{\bs \sigma_t} \le \mscr F \, , \quad \bs \sigma_t \bs u_t + \mscr F\,  \norm{\bs u_t} = 0
\end{align}
mit $\bs u_t \coloneqq \bs u - u_n \bs n$, dem tangentialen Anteil des Verschiebungsfeldes $\bs u$. Hierbei ist $\mscr F \ge 0$ eine Schranke für die Reibung. Wir können für $\bs u_t \not = 0$ aus der zweiten Gleichung von \eqref{eq:3.42} nachrechnen, dass gilt
\[
	hier rechnung
\]

Mit $\mscr F \coloneqq \mu \sigma_n$ erhalten wir das \textit{Reibungsgesetz von Coulomb}\index{Coulomb-Reibung}.
\end{bem}
