...
\item im weiteren wollen wir lineare Ansatzfunktionen verwenden. Wir bezeichnen, sofern nicht anders beschrieben, also im Folgenden $V_h$ mit
\[
	V_h \coloneqq \{v \in C^0(\Omega) \with v|_T \in \mcal P_1 \text{ für } T\in \mcal T_h, v|_{\partial \Omega} = 0\} \, .
\]

\item Beispiel, um zu sehen, dass auch für kleine Gitter der Rechenaufwand sehr hoch werden kann.
\begin{bsp}
Wir betrachten unser Variationsproblem \eqref{eq:2.9} auf $\Omega = [-1,1]^2$ mit $V_h$ wie oben eingeführt als den Raum der linearen Ansatzfunktionen auf einer Zerlegung $\mcal T_h$ wie unten aufgeführt.
...
\begin{align}\label{eq:2.12}
	...
	a(u_h,v_h) &= F(v_h) \quad \forall \, v_h \in V_h \, .
\end{align}
Da $V_h \subset H$ ist, gilt \eqref{eq:2.12} auch für alle $v_h\in V_h$. Ersetzen wir dies  in \eqref{eq:2.12} und subtrahieren \eqref{eq:2.12} und \eqref{eq:2.13}, so erhalten wir
...
\item \begin{bem*}
Die Gleichung \eqref{eq:2.14} drückt aus, dass die Verbindung $u-u_h$ orthogonal zum Raum $V_h$ steht und wird daher auch \textit{\idx{Galerkin-Orthogonalität}} genannt.
\end{bem*}
